...
public class QueryDslCustomerRepository implements CustomerRepository {
    ...
    public void setDataSource(DataSource dataSource) {
        ...
        customerAddressProjection = new Path[] { qCustomer.id, qCustomer.firstName, qCustomer.lastName, qCustomer.emailAddress, qAddress.id, qAddress.customerId, qAddress.street, qAddress.city, qAddress.country };
    }
    ...
    public List<Customer> findAll() {
        SQLQuery allCustomersQuery = qdslTemplate.newSqlQuery().from(qCustomer).leftJoin(qCustomer._addressCustomerRef, qAddress);
    ...
    private Customer findOne(Predicate predicate) {
        SQLQuery oneCustomerQuery = qdslTemplate.newSqlQuery().from(qCustomer).leftJoin(qCustomer._addressCustomerRef, qAddress).where(predicate);
        ...
        if (customer.getId() == null) {
            Long generatedKey = qdslTemplate.insertWithKey(qCustomer, new SqlInsertWithKeyCallback<Long>() {
                ...
                public Long doInSqlInsertWithKeyClause(SQLInsertClause insert) throws SQLException {
                    return insert.columns(qCustomer.firstName, qCustomer.lastName, qCustomer.emailAddress).values(customer.getFirstName(), customer.getLastName(), customer.getEmailAddress() == null ? null : customer.getEmailAddress().toString())
            ...
            qdslTemplate.update(qCustomer, new SqlUpdateCallback() {
                ...
                public long doInSqlUpdateClause(SQLUpdateClause update) {
                    return update.where(qCustomer.id.eq(customer.getId())).set(qCustomer.firstName, customer.getFirstName()).set(qCustomer.lastName, customer.getLastName())
                            .set(qCustomer.emailAddress, customer.getEmailAddress() == null ? null : customer.getEmailAddress().toString()).execute();
                }
            });
        }
        // save address data
        ...
        for (final Address a : customer.getAddresses()) {
            if (a.getId() != null) {
                qdslTemplate.update(qAddress, new SqlUpdateCallback() {
                    @Override
                    public long doInSqlUpdateClause(SQLUpdateClause update) {
                        return update.where(qAddress.id.eq(a.getId())).set(qAddress.customerId, customer.getId()).set(qAddress.street, a.getStreet()).set(qAddress.city, a.getCity()).set(qAddress.country, a.getCountry()).execute();
                    }
                });
            } else {
                qdslTemplate.insert(qAddress, new SqlInsertCallback() {
                    ...
                    public long doInSqlInsertClause(SQLInsertClause insert) {
                        return insert.columns(qAddress.customerId, qAddress.street, qAddress.city, qAddress.country).values(customer.getId(), a.getStreet(), a.getCity(), a.getCountry()).execute();
                    }
                });
            }
        }
